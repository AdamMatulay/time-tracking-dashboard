...
const appendItem = (item) => {
  ...
  if (dailyButton.classList.contains("active")) {
    elementTimeMap[item.title].textContent = item.timeframes.daily.current + "hrs";
    elementPreviousTimeMap[item.title].textContent = item.timeframes.daily.previous + "hrs";
    ...
    elementTimeMap[item.title].textContent = item.timeframes.weekly.current + "hrs";
    elementPreviousTimeMap[item.title].textContent = item.timeframes.weekly.previous + "hrs";
    previousText.forEach((el) => {
    ...
    elementTimeMap[item.title].textContent = item.timeframes.monthly.current + "hrs";
    elementPreviousTimeMap[item.title].textContent = item.timeframes.monthly.previous + "hrs";
    previousText.forEach((el) => {
...
fetch("/data.json")
  .then((response) => {
    if (!response.ok) return console.log("DATA.JSON LOADING FAILED!");
